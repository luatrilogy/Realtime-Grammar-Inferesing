...
// DATA
function nowCpuSeconds() {
    const usage = process.cpuUsage();
    return (usage.user + usage.system) / 1e6;
}
const startWall = perf_hooks_1.performance.now();
...
function readCorpusFromDir(dir, opts) {
    ...
    const chunks = [];
    for (const fp of all) {
        if (filesUsed.length >= opts.maxFiles)
            break;
        let buf;
        try {
...
if (!train || !out) {
    console.error("Usage: node bench.js --train DIR --out FILE [--lang bc|while|auto] [--time_budget SECONDS] [--sample N] [--max_mb MB] [--verbose]");
    process.exit(1);
}
...
if (!corpus.trim()) {
    console.error("No corpus text collected from training directory.\n" +
...
const res = (0, child_process_1.spawnSync)("python", [backendPy], {
    input: payload,
    encoding: "utf8",
...
const endWall = perf_hooks_1.performance.now();
