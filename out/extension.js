...
exports.generateFromGrammar = generateFromGrammar;
...
let previewTimer;
let currentProc;
let lastPreviewMetrics = undefined;
let vizPanel;
let lastMetrics = undefined;
...
function getcfg(key, def) {
    return vscode.workspace.getConfiguration('utaGrammarLab').get(key) ?? def;
}
function isGrammarFileName(fileName) {
    return /\.gram$/i.test(fileName) || fileName.toLowerCase().endsWith('.gramdsl');
}
function looksLikeGrammarText(text) {
    const head = text.slice(0, 4000);
    return /(^|\n)\s*start\s*:/i.test(head) && /(^|\n)\s*[A-Z][A-Za-z0-9_]*\s*->/.test(head);
}
async function showInNewGrammarDoc(grammarText, status = 'Inferred grammar') {
...
async function runAndShowInference(corpus, context) {
    const cfg = vscode.workspace.getConfiguration('utaGrammarLab');
    const py = cfg.get('pythonPath', 'python');
    const scriptRel = cfg.get('inferScript', 'python/infer_backend.py');
    const timeout = cfg.get('inferTimeoutSec', 10);
    ...
    const header = `// Inference metrics\n` +
        `// tokens: ${m.num_tokens ?? '?'}\n` +
        `// operators: ${ops || '(none)'}\n` +
        `// keywords: ${kws || '(none)'}\n\n`;
    ...
    vscode.window.setStatusBarMessage(`Grammar inferred • tokens=${m.num_tokens ?? '?'} • ops=[${ops}] • kw=[${kws}]`, 5000);
}
// run python script that speaks JSON over stdin/stdout
function runPythonJSON(pythonPath, scriptPath, payload, timeoutSec) {
    return new Promise((resolve, reject) => {
        ...
        catch { } ; reject(new Error(`Inference timed out after ${timeoutSec}s`)); }, Math.max(1, timeoutSec) * 1000);
        ...
        proc.on('close', _ => {
            ...
            try {
                ...
                if (json.error)
                    reject(new Error(json.error + (err ? `\n${err}` : '')));
                else
                    resolve(json);
            }
            catch (e) {
...
function extractQuotedTerminalsFromGrammar(gram) {
...
function rand(xs) { return xs[Math.floor(Math.random() * xs.length)]; }
...
function splitTopLevel(rhs, sep = '|') {
    const out = [];
    let buf = '';
...
function parseProductions(gram) {
    const rules = new Map();
...
function isRegex(tok) { return /^\/.*\/$/.test(tok); }
function isUpper(tok) { return /^[A-Z][A-Za-z0-9_]*$/.test(tok); }
function pickAlt(alts, lhs, depth) {
...
function minimalSampleFor(sym) {
    // Tail nonterminals should default to ε when we bail out
    if (sym === "AddExprTail" || sym === "TermTail" || sym === "RelExprTail" || sym === "BitExprTail")
        return "";
    if (sym === "Expr" || sym === "AddExpr" || sym === "Factor")
        return sampleNUM(); // e.g., "7"
    if (sym === "Stmt")
        return `${sampleID()} = ${sampleNUM()}`; // e.g., "x12 = 3"
    return sampleID();
}
// ---- MAIN GENERATOR ----
function generateFromGrammar(grammarText, startSymbol = "start", maxDepth = 6) {
    const rules = parseProductions(grammarText);
    function expand(sym, depth) {
        // terminals
        if (sym === "ε")
            return "";
        if (isQuoted(sym))
        ...
        if (depth > maxDepth) {
            ...
            if (["AddExprTail", "TermTail", "RelExprTail", "BitExprTail"].includes(sym))
                return "";
            return minimalSampleFor(sym);
        }
...
async function activate(ctx) {
    ...
    const clientOptions = { documentSelector: [{ language: 'gramdsl' }] };
    client = new node_1.LanguageClient('utaGrammarLsp', 'UTA Grammar LSP', serverOptions, clientOptions);
    await client.start(); // start the LSP
    ctx.subscriptions.push({ dispose: () => client?.stop() }); // add a Disposable
    ctx.subscriptions.push(vscode.commands.registerCommand('uta.gram.visualize', () => openVisualizer(ctx)), vscode.commands.registerCommand('uta.infer.preview', () => runInference(ctx, true)), vscode.commands.registerCommand('uta.infer.full', () => runInference(ctx, false)));
    const cfg = vscode.workspace.getConfiguration('utaGrammarLab');
    const pythonPath = cfg.get('pythonPath', 'python');
    const scriptRel = cfg.get('inferScript', 'python/infer_backend.py');
    const timeout = cfg.get('inferTimeoutSec', 10);
    const scriptAbs = path.join(ctx.extensionPath, scriptRel);
    const tracesRel = cfg.get('tracesDir') || 'examples';
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('utaGrammarLab.inferFromActive', async () => {
        const ed = vscode.window.activeTextEditor;
        if (!ed)
            return vscode.window.showWarningMessage('Open a text/code file first.');
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('utaGrammarLab.inferFromFile', async () => {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('utaGrammarLab.diffGrammars', async () => {
        // Try to use two currently-open grammar docs first
        const editors = vscode.window.visibleTextEditors
            .filter(e => e.document.languageId === 'gramdsl');
        let left = editors[0]?.document.uri;
        let right = editors[1]?.document.uri;
        ...
        await vscode.commands.executeCommand('vscode.diff', left, right, 'Grammar A ↔ Grammar B');
    }));
    ctx.subscriptions.push(vscode.commands.registerCommand('utaGrammarLab.checkCoverage', async () => {
        ...
        vscode.window.setStatusBarMessage(`Grammar coverage: ${coveragePct}% • uncovered lines: ${uncovered.length}/${totalLines}`, 5000);
    }));
    ctx.subscriptions.push(vscode.commands.registerCommand('utaGrammarLab.fuzzGrammar', async () => {
        // Step 1: pick an active grammar doc
        const grammarDoc = vscode.window.visibleTextEditors.find(e => e.document.languageId === 'gramdsl')?.document;
        if (!grammarDoc) {
...
async function openVisualizer(ctx) {
    vizPanel?.dispose();
    const panel = vscode.window.createWebviewPanel('gramViz', 'Grammar Visualizer', vscode.ViewColumn.Beside, {
        enableScripts: true,
...
async function runInference(ctx, isPreview) {
    const cfg = vscode.workspace.getConfiguration('utaGrammarLab');
    const pythonPath = cfg.get('pythonPath') || 'python';
    let inferScript = cfg.get('inferScript') || '';
    if (inferScript.includes('${extensionPath}')) {
        inferScript = inferScript.replace('${extensionPath}', ctx.extensionPath);
    }
    const tracesRel = cfg.get('tracesDir') || 'examples';
    const sample = cfg.get('previewSample') || 300;
    const budget = cfg.get('previewBudgetSeconds') || 0.5;
    ...
    currentProc = (0, child_process_1.spawn)(pythonPath, args, { cwd: folder.uri.fsPath });
    ...
    cp.on('close', async (code) => {
        ...
        try {
            ...
            if (isPreview &&
                ...
                vscode.window.setStatusBarMessage(`Preview kept (worse MDL: ${out.metrics.mdl.toFixed(2)} > ${lastPreviewMetrics.mdl.toFixed(2)})`, 3000);
                return;
            }
            ...
            vscode.window.setStatusBarMessage(`${isPreview ? 'Preview' : 'Full'} inference ✓ in ${ms} ms (coverage=${(out.metrics?.coverage ?? 0).toFixed(2)}, conflicts=${out.metrics?.conflicts ?? '?'})`, 5000);
        }
        catch (e) {
